...
object PIDs {
    ...
    private var list22d: Array<PIDStruct?>? = arrayOf(
        ...
        PIDStruct(0xffff, 1, false,-15f,35f,    -15f,   35f,    0.0f, 0f,"(m - a) * 0.1450777202","%01.1f","Boost",               "psi",    true, "Airflow.Default", ""),
        ...
        PIDStruct(0xf40C, 2, false,0f,  7000f,  -1f,    6000f,  0.0f, 0f,"x / 4",                 "%01.0f","Engine Speed",        "rpm",    true, "Default",         "r"),
        PIDStruct(0xf452, 1, false,0f,  100f,   -50f,   100f,   0.0f, 0f,"x / 2.55",              "%01.1f","Eth Content",         "%",      true, "",                ""),
        ...
        PIDStruct(0x1001, 1, false,-40f,55f,    -35f,   50f,    0.7f, 0f,"x * 0.75 - 48",         "%01.1f","IAT",                 "°C",     true, "Misc.Default",    ""),
        ...
        PIDStruct(0x200a, 2, true, 0f,  -5f,    -4f,    1000f,  0.7f, 0f,"x / 100",               "%01.2f","Knock Cyl 1",         "°",      true, "Ignition.Default","w"),
    ...
    private var list3Ed: Array<PIDStruct?>? = arrayOf(
        ...
        PIDStruct(0xFFFFFFFF, 1, false,-15f, 35f,    -15f,  35f,    0.0f, 0f,"(m - a) * 0.145077","%01.1f","Boost",               "psi",    true, "Airflow.Default", ""),
        ...
        PIDStruct(0xd0011db2, 2, false,0f,   100f,   -1000f,5000f,  0.0f, 0f,"x/655.36",          "%01.1f","DV Position",         "%",      true, "",                ""),
        ...
        PIDStruct(0xd0012400, 2, false,0f,   7000f,  -1.0f, 6000f,  0.0f, 0f,"x",                 "%01.0f","Engine Speed",        "rpm",    true, "Default",         ""),
        ...
        PIDStruct(0xd000c179, 1, false,-50f, 70f,    -20f,  50f,    0.9f, 0f,"(x - 64) / 1.33",   "%01.1f","IAT",                 "°C",     true, "Misc.Default",    ""),
        ...
        PIDStruct(0xd001988e, 1, false,0f,   -5f,    -3.0f, 3f,     0.8f, 0f,"(x - 128) / 2.66",  "%01.2f","Knock Avg",           "°",      true, "Ignition.Default",""),
    ...
    private var listDSGd: Array<PIDStruct?>? = arrayOf(
        PIDStruct(0x100c,     2, true, 0f,   600f,   -1000f,1000f,  0.0f, 0f,"x / 20",            "%01.1f","DSG Torque",          "nm",     true, "",               ""),
        PIDStruct(0x380c,     2, false,0f,   600f,   -100f, 10000f, 0.0f, 0f,"x",                 "%01.0f","Input Shaft 1",       "rpm",    true, "",               ""),
        PIDStruct(0x380d,     2, false,0f,   600f,   -100f, 10000f, 0.0f, 0f,"x",                 "%01.0f","Input Shaft 2",       "rpm",    true, "",               ""),
        PIDStruct(0x38eb,     2, false,0f,   20f,    -10f,  100f,   0.0f, 0f,"x / 100",           "%01.1f","Clutch 1 Press",      "bar",    true, "",               ""),
        PIDStruct(0x38ea,     2, false,0f,   20f,    -10f,  100f,   0.0f, 0f,"x / 100",           "%01.1f","Clutch 2 Press",      "bar",    true, "",               ""),
        PIDStruct(0x3835,     2, false,0f,   2000f,  -10f,  10000f, 0.0f, 0f,"x / 10",            "%01.0f","Clutch 1 Current",    "mA",     true, "",               ""),
        PIDStruct(0x383e,     2, false,0f,   2000f,  -10f,  10000f, 0.0f, 0f,"x / 10",            "%01.0f","Clutch 2 Current",    "mA",     true, "",               ""),
        PIDStruct(0x382e,     2, true, 0f,   600f,   -1000f,1000f,  0.0f, 0f,"x / 20",            "%01.0f","Clutch 1 Torque",     "nm",     true, "",               ""),
        PIDStruct(0x3838,     2, true, 0f,   600f,   -1000f,1100f,  0.0f, 0f,"x / 20",            "%01.0f","Clutch 2 Torque",     "nm",     true, "",               ""),
        PIDStruct(0x2104,     2, true, -40f, 150f,   -50f,  150f,   0.0f, 0f,"x",                 "%01.0f","Trans Temp",          "°C",     true, "",               ""),
    )
    ...
    fun loadDefaultPIDSDSG() {
        listDSG = listDSGd?.clone()
        buildDSGAssign()
        buildDSGData()
    }

    fun resetData(DSG: Boolean = false) {
        try {
            val pidData = if(DSG) dataDSG
            else getData()

            val pidList = if(DSG) listDSG
            else getList()

            for (i in 0 until pidData!!.count()) {
                val data = pidData[i]
                data?.let {
                    val pid = pidList!![i]
                    pid?.let {
    ...
    fun updateData(DSG: Boolean = false) {
        try {
            val pidData = if(DSG) dataDSG
            else getData()
    ...
    fun getDSGList(): Array<PIDStruct?>? {
        return listDSG
    }
    ...
    fun getDSGData(): Array<DATAStruct?>? {
        return dataDSG
    }
    ...
    fun getDSGAssign(): MutableMap<String, Int> {
        return assignDSG
    }
    ...
    fun buildAssign(mode: UDSLoggingMode = UDSLogger.getMode(), DSG: Boolean = false) {
        val pidList = if(DSG) listDSG
        else getList(mode)

        pidList?.let { list ->
            val assign = if(DSG) {
                assignDSG = mutableMapOf()
                assignDSG
            } else when (mode) {
                UDSLoggingMode.MODE_22 -> {
                    assign22 = mutableMapOf()
                    assign22
                }
                UDSLoggingMode.MODE_3E -> {
                    assign3E = mutableMapOf()
                    assign3E
                }
            }
            for (i in 0 until list.count()) {
    ...
    fun buildDSGAssign() {
        buildAssign(UDSLoggingMode.MODE_22, true)
    }

    fun buildData(mode: UDSLoggingMode = UDSLogger.getMode(), DSG: Boolean = false) {
        val pidList = if(DSG) listDSG
        else getList(mode)

        pidList?.let { list->
            val data = if(DSG) {
                dataDSG = arrayOfNulls(list.count())
                dataDSG
            } else when (mode) {
                UDSLoggingMode.MODE_22 -> {
                    data22 = arrayOfNulls(list.count())
                    data22
                }
                UDSLoggingMode.MODE_3E -> {
                    data3E = arrayOfNulls(list.count())
                    data3E
                }
            }
    ...
    fun buildDSGData() {
        buildData(UDSLoggingMode.MODE_22, true)
    }

    fun setList(mode: UDSLoggingMode = UDSLogger.getMode(), list: Array<PIDStruct?>?, dsg: Boolean = false) {
        list?.let {
            if(dsg) {
                listDSG = list
                buildData(mode, true)
    ...
    fun setDSGList(list: Array<PIDStruct?>?) {
        list?.let {
            listDSG = list
            buildDSGData()
        }
    ...
    fun getPID(address: Long, mode: UDSLoggingMode = UDSLogger.getMode(), DSG: Boolean = false): PIDStruct? {
        val pidList = if(DSG) listDSG
        else getList(mode)
